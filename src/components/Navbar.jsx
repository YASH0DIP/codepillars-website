...
export default function Navbar() {
  ...
  const navItems = ["Home", "About", "Services", "Portfolio", "Blogs", "Career", "Contact"];
  ...
  return (
    <nav className="absolute top-6 left-6 right-6 flex items-center justify-between px-8 py-4 rounded-full bg-linear-to-r from-[#FEE7C9] via-[#FFF4FF] to-[#E7EAEF] z-90 shadow-xl shadow-gray-500/10">
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`px-4 py-2 rounded-full transition-all duration-300 ${
              isActive(item.toLowerCase()) ? "bg-[#FDEED5]/90 shadow-xs shadow-slate-400" : "hover:text-orange-500"
            }`}
          >
            {item}
          </Link>
      ...
      {/* Desktop Button */}
      <button className="hidden lg:flex items-center gap-2 bg-slate-900 text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-slate-800 transition-all">
        Start Your Project <ArrowUpRight size={16} />
      </button>

      {/* Mobile Menu Icon */}
      <button className="lg:hidden p-2 hover:bg-orange-50/50 rounded-full cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <button className="col-span-2 mt-4 flex items-center justify-center gap-2 bg-slate-900 text-white w-full py-4 rounded-2xl font-semibold">
              Start Your Project <ArrowUpRight size={16} />
            </button>
          </motion.div>
        )}
